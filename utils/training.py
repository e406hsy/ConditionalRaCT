...
tf.compat.v1.set_random_seed(98765)

# from tensorflow.python import debug as tf_debug

from data_loaders import load_train_data, load_tr_te_data, get_num_items, load_user_info, load_unique_sid, load_item_info

from data_loaders import load_n2v
...
from models import (MultiVAE, MultiVAEWithUserinfo, WarpEncoder, WeightedMatrixFactorization, ProperlyShapedMultiDAE,
                    VWMF, GaussianVAE, MultiVAEWithPhase4WARP, LambdaRankEncoder, MultiVAEWithPhase4LambdaRank)
                    
my_path = '/content/gdrive/My Drive/RaCT_CF-master/RaCT_CF-master/'

dataset_with_user_info = ['ml-1m', 'ml-100k']

def get_n2v(data_subdir, mode = "node2vec"):
    data_dir = os.path.join(my_path, 'data', data_subdir)
    if data_subdir == 'ml-100k':
        if mode == "node2vec":
            n2v_vectors = load_n2v(os.path.join(data_dir,'node2vec_model_vectors'))
        elif mode == "node2vec_user_info":
            n2v_vectors = load_n2v(os.path.join(data_dir,'node2vec_userInfo_vectors'))
        else:
            n2v_vectors = None
        return n2v_vectors
    else:
...
def get_item_info(data_subdir):
    data_dir = os.path.join(my_path, 'data', data_subdir)
    if data_subdir == 'ml-1m':
        item_info = load_item_info(os.path.join(data_dir,'movies.dat'))
        return item_info
    else :
        return None

def get_user_info(data_subdir):
    pro_dir = os.path.join(my_path, 'data', data_subdir, 'pro_sg')
    if data_subdir in [ 'ml-1m', 'ml-100k'] :
        user_info = load_user_info(os.path.join(pro_dir,'users.csv'))
        return user_info
    else:
        return None

def get_unique_sid(data_subdir):
    pro_dir = os.path.join(my_path, 'data', data_subdir, "pro_sg")
    unique_sid = load_unique_sid(os.path.join(pro_dir,'unique_sid.txt'))
    return unique_sid

    
def write_plotting_data(arch_str, data_obj):
    plotting_dir = os.path.join(my_path, 'plotting_data', arch_str)
...
def get_data(data_subdir, mode="basic"):
    data_dir = os.path.join(my_path, 'data', data_subdir)
    pro_dir = os.path.join(data_dir, 'pro_sg')

    n_items = get_num_items(pro_dir)

    train_data, userId_map = load_train_data(os.path.join(pro_dir, 'train.csv'), n_items)
    vad_data_tr, vad_data_te, vad_userId_map = load_tr_te_data(os.path.join(pro_dir, 'validation_tr.csv'), os.path.join(pro_dir, 'validation_te.csv'),n_items)
    user_info = get_user_info(data_subdir) if data_subdir in dataset_with_user_info else None
    unique_sid = get_unique_sid(data_subdir) if data_subdir in dataset_with_user_info else None
    item_info = get_item_info(data_subdir) if data_subdir in dataset_with_user_info else None
    n2v_vectors = get_n2v(data_subdir, mode) if data_subdir == 'ml-100k' else None
    return {
        'n_items': n_items,
        ...
        'vad_userId_map': vad_userId_map,
        'unique_sid' : unique_sid,
        'n2v_vectors' : n2v_vectors,
    }


def get_test_data(data_subdir, mode='basic'):
    data_dir = os.path.join(my_path, 'data', data_subdir)
    pro_dir = os.path.join(data_dir, 'pro_sg')
    ...
    train_data, userId_map = load_train_data(os.path.join(pro_dir, 'train.csv'), n_items)
    test_tr, test_te, test_userId_map = load_tr_te_data(os.path.join(pro_dir, 'test_tr.csv'), os.path.join(pro_dir, 'test_te.csv'), n_items)
    user_info = get_user_info(data_subdir) if data_subdir in dataset_with_user_info else None
    n2v_vectors = get_n2v(data_subdir, mode) if data_subdir == 'ml-100k' else None
    return {
        'n_items': n_items,
        ...
        'test_te': test_te,
        'user_info': user_info,
        'test_userId_map': test_userId_map,
        'train_data' : train_data,
        'userId_map' : userId_map,
        'n2v_vectors' : n2v_vectors,
    }
...
def get_model(batch_of_users, heldout_batch, model_class=None, **kwargs):
    ...
    elif model_class == 'multi_vae_with_userinfo':
        constructor = MultiVAEWithUserinfo
...
def make_summary_writer(arch_str):
    ...
    """
    log_dir = os.path.join(my_path, 'logging', arch_str)
...
def make_checkpoint_dir(arch_str):
    chkpt_dir = os.path.join(my_path, 'checkpoints', arch_str)
...
def train(model_class=None,
    ...
    assert mode in ["basic", "one_hot", "node2vec", "node2vec_user_info"]
    assert data_subdir in ['ml-100k', 'ml-20m', 'netflix-prize', 'msd', 'ml-1m']
    assert model_class in ['multi_vae', 'warp_encoder', 'multi_vae_with_userinfo','wmf', 'new_multi_dae', 'variational_wmf', 'gaussian_vae', 'phase_4_warp', 'lambdarank_actor', 'phase_4_lambdarank']

    train_args = locals()
    model_with_userinfo = ['multi_vae_with_userinfo']
    ...
    data_dict = get_data(data_subdir, mode)
    ...
    if model_class in model_with_userinfo:
        if mode == "node2vec":
            print("node2vec mode")
            assert type(n2v_vectors) != type(None)
        training_dataset = train_dataset(train_data, batch_size, mode = mode, user_info = user_info, userId_map = userId_map, n2v_vectors = n2v_vectors)
        validation_dataset = tr_te_dataset(vad_data_tr, vad_data_te, batch_size, mode = mode, user_info = user_info, vad_userId_map= vad_userId_map, n2v_vectors = n2v_vectors, train_data = train_data, userId_map = userId_map)
...
def test(
    ...
    assert mode in ["basic", "one_hot", "node2vec", "node2vec_user_info"]
    assert data_subdir in ['ml-20m', 'netflix-prize', 'msd', 'ml-1m', 'ml-100k']
    assert model_class in ['multi_vae', 'warp_encoder', 'multi_vae_with_userinfo','wmf', 'new_multi_dae', 'variational_wmf', 'gaussian_vae', 'phase_4_warp', 'lambdarank_actor', 'phase_4_lambdarank']
    ...
    data_dict = get_test_data(data_subdir, mode)

    n_items = data_dict['n_items']
    test_tr = data_dict['test_tr']
    test_te = data_dict['test_te']
    user_info = data_dict['user_info']
    test_userId_map = data_dict['test_userId_map']
    train_data = data_dict['train_data']
    n2v_vectors = data_dict['n2v_vectors']
    userId_map = data_dict['userId_map']
    
    if model_class in model_with_userinfo:
        testing_dataset = tr_te_dataset(test_tr, test_te, batch_size,mode = mode, user_info= user_info, vad_userId_map = test_userId_map,
                                        n2v_vectors = n2v_vectors, train_data = train_data, userId_map = userId_map)
    ...
    with tf.Session() as sess:
        ...
        try:
            ...
            while True:
                ...
                ndcg100_list.append(
                    NDCG_binary_at_k_batch(
                        pred_val, heldout_batch[:,:n_items], k=100, input_batch=batch_of_users[:,:n_items]))
                recall50_list.append(
                    Recall_at_k_batch(pred_val, heldout_batch[:,:n_items], k=50, input_batch=batch_of_users[:,:n_items]))
                recall20_list.append(
                    Recall_at_k_batch(pred_val, heldout_batch[:,:n_items], k=20, input_batch=batch_of_users[:,:n_items]))
                ndcg200_list.append(
                    NDCG_binary_at_k_batch(
                        pred_val, heldout_batch[:,:n_items], k=200, input_batch=batch_of_users[:,:n_items]))
                ndcg5_list.append(
                    NDCG_binary_at_k_batch(
                        pred_val, heldout_batch[:,:n_items], k=5, input_batch=batch_of_users[:,:n_items]))
                ndcg3_list.append(
                    NDCG_binary_at_k_batch(
                        pred_val, heldout_batch[:,:n_items], k=3, input_batch=batch_of_users[:,:n_items]))
                ndcg1_list.append(
                    NDCG_binary_at_k_batch(
                        pred_val, heldout_batch[:,:n_items], k=1, input_batch=batch_of_users[:,:n_items]))
                
                dcg100_list.append(
                    NDCG_binary_at_k_batch(
                        pred_val, heldout_batch[:,:n_items], k=100, input_batch=batch_of_users[:,:n_items], normalize=False))
