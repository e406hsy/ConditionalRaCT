...
def maybe_download_and_extract_movie_data_100k(data_dir, force_overwrite=False):
...
def maybe_download_and_extract_movie_data_1m(data_dir, force_overwrite=False):
    write_path = os.path.join(data_dir, 'ml-1m.zip')
    zip_url = "http://files.grouplens.org/datasets/movielens/ml-1m.zip"
    if not os.path.isfile(write_path):
        os.makedirs(data_dir, exist_ok=True)
        print("Zip not downloaded. Downloading now...")
    ...
    extract_destination = os.path.join(data_dir, "ml-1m")
    if os.path.isdir(extract_destination):
        if not force_overwrite:
            print("seems extracted datadir already exists, and not forcing overwrite. Exiting.")
            return
    ...
    current_dir = os.path.join(data_dir, 'ml-1m', 'ml-1m')
    temp_dir = os.path.join(data_dir, 'ml-1m-temp')
    right_dir = os.path.join(data_dir, 'ml-1m')
...
def process_unzipped_data_ml_1m(DATA_DIR,
                          force_overwrite=False,
                          n_heldout_users=100,
                          discard_ratings_below=3.5,
                          min_users_per_item_to_include=0,
    ...
    user_info = pd.read_csv(os.path.join(DATA_DIR, 'users.dat'),delimiter = r'::', names =['userId','gender','age','occupation','zip_code'], engine = 'python')
    user_info.to_csv(os.path.join(pro_dir,'users.csv'),index=False)
    
    raw_data = pd.read_csv(os.path.join(DATA_DIR, 'ratings.dat'), delimiter=r'::', names=['userId', 'movieId', 'rating', 'timestamp'], engine='python')
    ...
    save_dict_to_file(show2id, os.path.join(pro_dir,"unique_sid_dictionary"))
    # In[16]:
    ...
    def numerize(tp):
        ...
        uid = map(lambda x: profile2id[x], tp['userId'])
        sid = map(lambda x: show2id[x], tp['movieId'])
        return pd.DataFrame(data={'userId':userId, 'uid': list(uid), 'sid': list(sid)}, columns=['userId','uid', 'sid'])
...
def process_unzipped_data_ml_100k(DATA_DIR,
                          force_overwrite=False,
                          n_heldout_users=100,
                          discard_ratings_below=3.5,
                          min_users_per_item_to_include=0,
    ...
    if os.path.isdir(pro_dir):
        if force_overwrite:
            ...
            print("pro_sg dir already exists. Exiting.")
            return
    raw_data = pd.read_csv(os.path.join(DATA_DIR, 'u.data'), delimiter='\t', names=['userId', 'movieId', 'rating', 'timestamp'], engine='python')
    ...
    user_info = pd.read_csv(os.path.join(DATA_DIR, 'u.user'),delimiter = r'|', names =['userId','gender','age','occupation','zip_code'], engine = 'python')
    user_info.to_csv(os.path.join(pro_dir,'users.csv'),index=False)
...
if __name__ == '__main__':
    ...
    assert dataset in ['ml-100k', 'ml-20m', 'ml-1m', 'netflix-prize', 'msd', 'all']
    if dataset == 'ml-100k' or dataset == 'all':
        print("Doing ml-100k stuff!!")
        maybe_download_and_extract_movie_data_100k("./data", force_overwrite=force_overwrite)
        process_unzipped_data_ml_100k('./data/ml-100k', force_overwrite=force_overwrite)
    
    if dataset == 'ml-1m' or dataset == 'all':
        print("Doing ml-1m stuff!")
        maybe_download_and_extract_movie_data_1m("./data", force_overwrite=force_overwrite)
        process_unzipped_data_ml_1m('./data/ml-1m', force_overwrite=force_overwrite)
